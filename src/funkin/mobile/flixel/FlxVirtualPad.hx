...
class FlxVirtualPad extends FlxMobileInputManager
{
	...
	private function createButton(X:Float, Y:Float, Width:Int, Height:Int, Graphic:String, ?Color:Int = 0xFFFFFF):FlxButton
	{
		...
		button.frames = FlxTileFrames.fromFrame(Paths.sparrowAtlas('virtualpad').getByName(Graphic), FlxPoint.get(Width, Height));
		...
		button.color = Color;
		button.antialiasing = Settings.data.antialiasing;
