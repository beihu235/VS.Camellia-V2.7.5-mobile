...
class FlxStrip extends FlxSprite
{
	...
	override public function draw():Void
	{
		...
		for (camera in cameras)
		{
			...
			#if !flash
			camera.drawTriangles(graphic, vertices, indices, uvtData, colors, _point, blend, wrapMode, antialiasing, colorTransform, shader);
			#else
			camera.drawTriangles(graphic, vertices, indices, uvtData, colors, _point, blend, wrapMode, antialiasing);
			#end
		}
	}
}
