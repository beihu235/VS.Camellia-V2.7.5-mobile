...
class FlxDrawTrianglesItem extends FlxDrawBaseItem<FlxDrawTrianglesItem> {
	...
	public function addTriangles(vertices:DrawData<Float>, indices:DrawData<Int>, uvtData:DrawData<Float>, ?colors:DrawData<Int>, ?position:FlxPoint, ?cameraBounds:FlxRect, ?transform:ColorTransform):Void {
		if (position == null)
			position = point.set();
		...
		while (i < verticesLength) {
			...
			else inflateBounds(bounds, tempX, tempY);
		...
		var alphaTransform:Float = transform != null ? transform.alphaMultiplier : 1.0;
		for (_ in 0...indices.length) alphas.push(alphaTransform);
