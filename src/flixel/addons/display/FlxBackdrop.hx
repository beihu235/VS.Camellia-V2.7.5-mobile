...
class FlxBackdrop extends FlxSprite
{
	...
	override function drawSimple(camera:FlxCamera):Void
	{
		...
		for (tileX in 0...tilesX)
		{
			for (tileY in 0...tilesY)
			{
				...
				camera.copyPixels(frame, pixels, pixels.rect, _flashPoint, colorTransform, blend, antialiasing, wrapMode);
			}
		}
	...
	override function drawComplex(camera:FlxCamera)
	{
		...
		if (FlxG.renderTile)
		{
			...
			drawItem = camera.startQuadBatch(graphic, isColored, hasColorOffsets, blend, antialiasing, wrapMode, shader);
		}
		else
		{
			camera.buffer.lock();
		...
		for (tileX in 0...tilesX)
		{
			for (tileY in 0...tilesY)
			{
				...
				{
					final pixels = drawBlit ? _blitGraphic.bitmap: framePixels;
					camera.drawPixels(frame, pixels, mat, colorTransform, blend, antialiasing, wrapMode, shader);
				}
				else
				{
