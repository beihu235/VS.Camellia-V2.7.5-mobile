...
class FlxCamera extends FlxBasic
{
	...
	public function drawNoteVertices(?frame:FlxFrame, vertices:Array<Float>, ?transform:ColorTransform, ?blend:BlendMode, ?smoothing:Bool = false, ?customColoring:Bool = false, ?stealthTop:Float, ?stealthBottom:Float):Void
	{
		if (FlxG.renderBlit) // don't feel like adding support for this
			return;

		var drawItem = startNoteBatch(frame.parent, blend, smoothing, customColoring);
		drawItem.addVertices(frame, vertices, transform, stealthTop, stealthBottom);
	}
